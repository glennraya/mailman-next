...
over the same two ports, so `mailman service install` refuses rather than join
...
| `mailgun` | `multipart/form-data`: `recipient`, `sender`, `from`, `subject`, `body-plain`, `body-html`, `stripped-text`, `stripped-signature`, `message-headers`, `domain`, `attachment-count`, plus **every MIME header as its own field** (`Message-Id`, `In-Reply-To`, `References`, …) and `X-Mailgun-Incoming: Yes` | `signature`, `timestamp` and `token` **in the body**, where `signature` is `HMAC-SHA256(key, timestamp + token)`, which is what Mailgun's own verification helpers recompute |
